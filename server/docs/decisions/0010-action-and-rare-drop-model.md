...
## Status
Accepted (the `hacksilver_min`/`hacksilver_max` reward columns on `game.action` described below are superseded by [0011](0011-generalized-resource-and-primary-reward-model.md); the `game.action_rare_drop` decision remains in effect, but `item_key`'s "no foreign key yet" placeholder is resolved by [0012](0012-item-catalog-model.md))
